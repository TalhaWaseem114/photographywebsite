  ...
  return (
    <>
      ...
      >
        ...
        <div className="flex items-center gap-3">
          <Link href="/" className="flex items-center gap-2 group">
            <span className="text-lg font-medium tracking-[0.15em] uppercase font-display" style={{ fontFamily: "'Antonio', sans-serif" }}>
              <span className="text-muted group-hover:text-foreground transition-colors duration-300">.</span>HZ
            </span>
            <div className="flex flex-col ml-2 border-l border-divider pl-2">
              <span className="text-[10px] tracking-[0.25em] uppercase font-medium text-foreground/90">Hanzala</span>
              <span className="text-[7.5px] tracking-[0.3em] uppercase text-muted mt-0.5">Photography</span>
            </div>
          </Link>
        </div>
            ...
            return (
              <Link
                ...
                className={`text-[12px] tracking-[0.28em] transition-all duration-300 font-medium ${
                  isActive
                    ? 'text-foreground border-b border-foreground/60 pb-1.5'
        ...
        >
          <div className={`h-[1px] bg-foreground transition-all duration-350 ${menuOpen ? 'w-full rotate-45 translate-y-[3.8px]' : 'w-full'}`}></div>
          <div className={`h-[1px] bg-foreground transition-all duration-350 ${menuOpen ? 'w-full -rotate-45 -translate-y-[3.8px]' : 'w-4/5 group-hover:w-full'}`}></div>
        </button>
      </header>
      ...
      <div
        className={`fixed inset-0 z-50 bg-background/80 backdrop-blur-md flex justify-end transition-all duration-500 ease-in-out
          ${menuOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
        onClick={() => setMenuOpen(false)}
      >
        <div
          className={`w-full max-w-[480px] bg-panel h-full border-l border-divider flex flex-col justify-between p-8 sm:p-12 md:p-16 transition-transform duration-500 ease-out
            ${menuOpen ? 'translate-x-0' : 'translate-x-full'}`}
          onClick={(e) => e.stopPropagation()}
        >
          {/* Header Area inside overlay */}
          <div className="flex justify-between items-center select-none pt-4">
            <span className="text-[12px] font-mono tracking-[0.25em] text-muted uppercase">
              INDEX LEDGER
            </span>
          </div>
              ...
              return (
                <Link
                  ...
                  className={`font-display text-[22px] sm:text-[28px] tracking-[0.18em] uppercase flex items-center group transition-all duration-300 w-fit
                    ${isActive ? 'text-[var(--accent)] font-semibold' : 'text-muted hover:text-foreground hover:translate-x-1.5'}`}
                  style={{ fontFamily: "'Antonio', sans-serif" }}
                >
                  <span className={`text-[9px] font-mono mr-4 select-none transition-colors
                    ${isActive ? 'text-[var(--accent)]/50' : 'text-foreground/10 group-hover:text-[var(--accent)]/50'}`}>
                    0{idx + 1}
                  </span>
                  {item.name}
                </Link>
          ...
          <div className="flex flex-col gap-6 text-left border-t border-divider pt-8">
            <div className="flex flex-col gap-1 text-[8.5px] font-mono text-muted uppercase tracking-[0.18em]">
            ...
            <div className="flex items-center gap-4.5 select-none">
              ...
              <a 
                ...
                className="w-8 h-8 rounded-full bg-foreground/5 border border-divider flex items-center justify-center text-muted hover:text-[var(--accent)] hover:border-[var(--accent)]/40 hover:bg-[var(--accent)]/5 transition-all duration-300"
              >
