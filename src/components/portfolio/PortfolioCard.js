...
export default function PortfolioCard({ item, onClick, priority = false }) {
  ...
  return (
    <div
      onClick={onClick}
      className={`group relative rounded-3xl border border-white/[0.04] bg-[#0c0c0e] cursor-pointer hover:border-[var(--accent)]/30 transition-all duration-500 ease-out`}
      style={{
        boxShadow: '0 4px 30px rgba(0, 0, 0, 0.4)',
      }}
    >
      {/* Top-Center Category Notch (Mobile notch style) */}
      <div className="absolute -top-[8px] left-1/2 -translate-x-1/2 z-30 flex flex-col items-center">
        {/* Dynamic Notch Tab Extender matching category label width & color */}
        <div 
          className="w-full h-[8px] border-t border-x border-white/10 rounded-t-lg transition-opacity duration-300 pointer-events-none -mb-[1px]" 
          style={{ backgroundColor: 'rgba(10, 10, 12, 0.55)', backdropFilter: 'blur(12px)', WebkitBackdropFilter: 'blur(12px)' }}
        />
        
        <span 
          className="border-b border-x border-white/10 text-[var(--accent)] text-[8px] font-condensed tracking-[0.22em] uppercase px-3.5 pt-0.5 pb-1.5 rounded-b-xl shadow-md block select-none"
          style={{ backgroundColor: 'rgba(10, 10, 12, 0.55)', backdropFilter: 'blur(12px)', WebkitBackdropFilter: 'blur(12px)' }}
        >
          {item.category}
        </span>
      ...
      <div className="absolute inset-0 z-20 border border-[var(--accent)]/0 group-hover:border-[var(--accent)]/20 rounded-3xl transition-all duration-500 pointer-events-none" />

      {/* Image Container */}
      <div className={`relative w-full ${getAspectClass(item.aspectRatio)} overflow-hidden rounded-[22px]`}>
        <Image
          src={item.src}
          alt={item.title}
          fill
        ...
        {/* Instagram-style Multi-image Overlay Indicator (Moved to bottom right, fades out on hover) */}
        {item.slides && item.slides.length > 1 && (
          <div className="absolute bottom-3.5 right-3.5 z-20 bg-black/60 border border-white/10 backdrop-blur-xs p-1.5 rounded flex items-center justify-center text-white/50 group-hover:opacity-0 group-hover:scale-75 transition-all duration-300 pointer-events-none">
