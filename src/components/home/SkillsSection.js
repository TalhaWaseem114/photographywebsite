...
export default function SkillsSection() {
  ...
  return (
    <section id="skills-section" className="relative min-h-[450px] w-full bg-background border-t border-divider">
      ...
      <div className="w-full h-full flex flex-col lg:flex-row items-start px-8 py-20 pl-8 md:pl-28 lg:pl-32 lg:pr-16 xl:pr-24 gap-12 lg:gap-16">
        ...
        <div className="relative w-fit flex-shrink-0 select-none pt-2">
          <span className="font-display text-[90px] md:text-[120px] text-foreground/10 font-extralight leading-none">
            03
          </span>
          {/* Diagonal Slash Line */}
          <div className="absolute left-[30px] bottom-[30px] w-[140px] h-[1px] bg-foreground/20 -rotate-[35deg] transform origin-bottom-left"></div>
        </div>
        ...
        <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-8 gap-y-12 w-full sm:divide-x-0 lg:divide-x lg:divide-divider">
          {skills.map((skill, index) => (
            ...
            >
              ...
              <div className="mb-6 flex flex-col gap-1.5 select-none">
                ...
                <h4 className="text-[13px] font-display font-light tracking-[0.15em] uppercase text-foreground/90 group-hover:text-foreground transition-colors">
                  {skill.title}
                </h4>
              </div>

              {/* Skill Sub-items List */}
              <ul className="text-muted text-[12px] font-light tracking-wide space-y-3 mb-10 flex-1">
                {skill.items.map((item, i) => (
                  <li key={i} className="flex items-center gap-2.5">
                    <span className="w-[3px] h-[3px] bg-[#c5a075]/60 rounded-full block flex-shrink-0"></span>
                    <span className="hover:text-foreground/80 transition-colors">{item}</span>
                  </li>
                ))}
              </ul>
              ...
              <div className="mt-auto flex items-center gap-4 w-full select-none">
                ...
                <span className="text-[11px] font-condensed font-light tracking-wider text-muted min-w-[28px] text-right">
                  {skill.progress}%
                </span>
              </div>
