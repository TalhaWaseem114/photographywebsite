...
export default function TestimonialsSection() {
  ...
  return (
    <section id="testimonials-section" className="relative min-h-[400px] w-full bg-black overflow-hidden border-t border-white/10">

      {/* ─── Main Content Grid ─── */}
      {/* 4 columns on large screens: 1 for Header info, 3 for testimonials */}
      <div className="w-full h-full grid grid-cols-1 lg:grid-cols-4 px-8 py-20 pl-8 md:pl-24 lg:pl-24 lg:pr-16 xl:pr-24 gap-12 lg:gap-0 lg:divide-x lg:divide-white/10">
        ...
        <div className="flex flex-col justify-between pr-0 lg:pr-8 h-full min-h-[160px] lg:min-h-auto">
          <div>
            ...
            <span className="font-condensed text-[12px] tracking-[0.4em] uppercase text-white/40 mb-3 block">
              KIND WORDS
            </span>
            {/* Big stacked heading layout */}
            <h2 className="font-display text-[32px] md:text-[38px] leading-[1.05] tracking-[0.05em] uppercase text-white font-light">
          ...
          <div className="flex items-center gap-6 mt-8 lg:mt-0 select-none">
            <button className="text-white/30 hover:text-white transition-colors duration-300 group">
        ...
        {testimonials.map((item, index) => (
          <div
            key={index}
            className="flex flex-col justify-between h-full pt-2 lg:pt-0 lg:pl-8 xl:pl-10 lg:pr-4 first:pl-0 border-t border-white/5 lg:border-t-0"
          >
            ...
            <p className="text-[13px] leading-[1.9] text-white/60 font-light tracking-wide mb-8 max-w-[340px] lg:max-w-none">
              {item.quote}
            </p>

            {/* Bottom Anchor: Author Meta Layout and Gold Accent Line */}
            <div className="mt-auto flex flex-col gap-1.5 select-none pb-1">
              <h4 className="text-[12px] font-condensed font-medium tracking-[0.18em] text-white/90">
                {item.author}
              </h4>
